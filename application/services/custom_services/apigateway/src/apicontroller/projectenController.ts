...
export class projectenController implements Controller {
    ...
    private initializeRoutes() {
        this.router.delete('/projecten/:id', this.GpDelete);
this.router.get('/projecten/get/search', this.GpSearch);
this.router.put('/projecten', this.GpUpdate);
this.router.put('/projecten/get/update', this.GpSearchForUpdate);
this.router.get('/projecten', this.GpGetAllValues);
this.router.post('/projecten', this.GpCreate);
    }

public GpDelete(req: Request, res: Response) {
            new CustomLogger().showLogger('info', 'Enter into projectenController.ts: GpDelete');
        new ApiAdaptar().delete(Constant.PROJECTSURL + `${req.url}` )
...
public GpSearch(req: Request, res: Response) {
        ...
        new ApiAdaptar().get(Constant.PROJECTSURL + `${req.url}` )
        .then((res: any) => res.response.json()).then(result => {
              req.baseUrl === '/mobile' ? res.send(result) :
              req.baseUrl === '/desktop' ? res.send(result) : res.send(null)
...
public GpUpdate(req: Request, res: Response) {
        ...
        new ApiAdaptar().put(Constant.PROJECTSURL + `${req.url}` , req.body)
        .then((res: any) => res.response.json()).then(result => {
              req.baseUrl === '/mobile' ? res.send(result) :
              req.baseUrl === '/desktop' ? res.send(result) : res.send(null)
...
public GpCreate(req: Request, res: Response) {
        ...
        new ApiAdaptar().post(Constant.PROJECTSURL + `${req.url}` , req.body)
        .then((res: any) => res.response.json()).then(result => {
              req.baseUrl === '/mobile' ? res.send(result) :
              req.baseUrl === '/desktop' ? res.send(result) : res.send(null)
